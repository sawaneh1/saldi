...
{
	error_log('opret_email_send: ' . $message . ' (ip ' . if_isset($_SERVER, '?', 'REMOTE_ADDR') . ')');
}
...
if ($key === '' || !hash_equals($forventet_key, $key)) {
	opret_email_log('rejected key for package "' . $pakke . '"');
...
if (!$resultat['success']) {
	opret_email_log('could not send to "' . $email . '" for package "' . $pakke . '": ' . $resultat['error']);
...
opret_email_log('sent to "' . $email . '" for package "' . $pakke . '"');
