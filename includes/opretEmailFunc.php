...
if (!function_exists('opret_email_render')) {
	...
	function opret_email_render($html, $package, $vars = array())
	{
		...
		foreach (array_keys(opret_email_placeholders()) as $name) {
			// Both {{navn}} and {{ navn }} are accepted so a stray space typed
			// in the source view doesn't leave a raw placeholder in the mail.
			$search[] = '/\{\{\s*' . preg_quote($name, '/') . '\s*\}\}/';
			$replace[] = htmlspecialchars(if_isset($values, '', $name), ENT_QUOTES, 'UTF-8');
		}
...
if (!function_exists('opret_email_build')) {
	...
	function opret_email_build($kode, $vars = array(), $sprog = 1)
	{
		...
		$emne = strip_tags(opret_email_render($settings['emne'], $package, $vars));
